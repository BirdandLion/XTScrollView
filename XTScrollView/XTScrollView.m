...
@interface XTScrollView() <UIScrollViewDelegate>
...
-(void)tapCallback
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(scrollViewDidClickPage:index:)])
    {
        [self.delegate scrollViewDidClickPage:self index:self.pageControl.currentPage];
    }
}
...
{
    // 如果是2张一下的图片, 改方法执行一次
    if (self.isFirstLoadImage && (ImageViewCount < 3)) return;
    ...
    // 如果是显示一张, 那么隐藏pageControl
    self.pageControl.hidden = (ImageViewCount == 1) ? YES : NO;
}

- (void)displayNextImage
{
    if (ImageViewCount <= 1) return;
    
    if (ImageViewCount == 2) { // 2张图片
        int index = (self.pageControl.currentPage == 0) ? 1 : 0;
        if (self.isScrollDirectionPortrait) {
            [self.scrollView setContentOffset:CGPointMake(0, index * self.scrollView.frame.size.height) animated:YES];
        } else {
...
- (void)startTimer
{
    if (!self.stopAutoScroll) {
        NSTimer *timer = [NSTimer timerWithTimeInterval:self.delayTime target:self selector:@selector(displayNextImage) userInfo:nil repeats:YES];
